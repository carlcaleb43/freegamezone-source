...
#if defined(NDEBUG)
# error "FreeGameZone cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2d83ce9e52bc3af37b98b0701b7133bf0e750538d5815f7dadd2aae83ebaff79");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FreeGameZone: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FreeGameZone Signed Message:\n";
...
{
    // FreeGameZone: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 8 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 31342864 * COIN;
            }

    // Subsidy is cut in half every 839541 blocks
    nSubsidy >>= (nHeight / 839541);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FreeGameZone: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FreeGameZone: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1477848608, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "fgfhgh";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 8 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040c9bde987f1b5c9b5a7b817904a0c7e307799b9aa9fa30e52112adc7d3c2380fbedf399900042d7a151f7dd214465db191ef825e63246eea17f98681caf719a4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1477848608;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2303395;
        ...
        assert(block.hashMerkleRoot == uint256("0xae48466136fe7f5513eeaaa3da92b7011fdee664d55563db6b82ce43babde203"));
...
unsigned char pchMessageStart[4] = { 0x3d, 0x09, 0x06, 0x43 }; // FreeGameZone: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FreeGameZoneMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FreeGameZoneMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FreeGameZoneMiner(CWallet *pwallet)
{
    printf("FreeGameZoneMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("freegamezone-miner");
    ...
    try { loop {
        ...
        printf("Running FreeGameZoneMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FreeGameZoneMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FreeGameZoneMiner, pwallet));
}
